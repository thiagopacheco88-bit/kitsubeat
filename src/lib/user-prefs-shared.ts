/**
 * Client-safe pieces of user-prefs (constants + types).
 *
 * Split out from user-prefs.ts so that client components can import
 * PLACEHOLDER_USER_ID etc. without webpack tracing into getCurrentUserId,
 * which dynamically imports @clerk/nextjs/server (server-only).
 *
 * Server callers keep importing from "@/lib/user-prefs" — that module
