...
// ---------------------------------------------------------------------------
// Captain Tsubasa characters real players article
// ---------------------------------------------------------------------------

const TSUBASA_REAL_SLUG = "captain-tsubasa-characters-real-players";
const TSUBASA_REAL_URL = `/journal/${TSUBASA_REAL_SLUG}`;

test.describe("Journal article — captain-tsubasa-characters-real-players", () => {
  test.describe.configure({ timeout: 90_000 });

  test.beforeAll(async ({ browser }) => {
    const page = await browser.newPage();
    await page.goto(TSUBASA_REAL_URL, { waitUntil: "domcontentloaded", timeout: 80_000 });
    await page.close();
  });

  test("loads without 500 error, shows heading, and hero image is visible", async ({ page }) => {
    const response = await page.goto(TSUBASA_REAL_URL);
    expect(response?.status()).not.toBe(500);
    expect(response?.status()).toBe(200);
  ...
  test("vocab table renders as HTML table, not raw markdown pipes", async ({ page }) => {
    await page.goto(TSUBASA_REAL_URL);
  ...
  test("FAQ section renders with expected questions", async ({ page }) => {
    ...
    const articleText = await page.locator("article").innerText({ timeout: 10_000 });
    expect(articleText).toContain("Is Roberto Hongo based on a real player?");
    expect(articleText).toContain("Who is Karl Heinz Schneider based on");
    expect(articleText).toContain("Is there a real Nankatsu SC club?");
    expect(articleText).toContain("Who is Pierre based on in Captain Tsubasa?");
  });

  test("content sections render - Tostao, Socrates, Rummenigge, Platini, Nankatsu", async ({ page }) => {
    await page.goto(TSUBASA_REAL_URL);
    await page.locator("article").waitFor({ timeout: 10_000 });

    const articleText = await page.locator("article").innerText();
    expect(articleText).toContain("Tostao");
    expect(articleText).toContain("Socrates");
    expect(articleText).toContain("Rummenigge");
    expect(articleText).toContain("Platini");
    expect(articleText).toContain("Nankatsu");
    expect(articleText).toContain("Roberto Hongo");
    expect(articleText).toContain("Sepp Maier");
    expect(articleText).toContain("FC Barcelona");
  });
