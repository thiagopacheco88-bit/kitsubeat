/**
 * Phase 14.1 AC #3 — /path visual diff light theme.
 *
 * Captures /path at 390x844 light theme, compares against the locked
 * Playwright snapshot (path-light.png) at 2% pixel-diff tolerance.
 *
...
test.describe("/path visual diff (light theme)", () => {
  test("matches snapshot at 2% tolerance (AC #3)", async ({ page, context }) => {
    await page.setViewportSize({ width: 390, height: 844 });

    // Set light theme via cookie (matches SSR theme-cookie pattern from theme-toggle.spec.ts)
    ...
    await expect(page).toHaveScreenshot("path-light.png", {
