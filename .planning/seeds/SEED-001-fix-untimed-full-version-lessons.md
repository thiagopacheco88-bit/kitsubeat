...
- **`pokemon-getto-da-ze-rica-matsumoto` needs a correct yt_id BEFORE this seed runs.** Current yt_id (`hMKf5mE3sdo`) is the Mezase TFT video, not Getto. Lesson lyrics confirm Getto is a real distinct song with its own verse content ("ポケモンゲットだぜ / 飛び出せポケモン / めざすはチャンピオン / みんなともだちさ" — 15 verses). When SEED-001 triggers, the prep step is to find a correct YT upload for this song first, then let the pipeline regenerate timing against the right audio. Without the yt_id fix, the pipeline would generate timings against the wrong audio (Mezase) and produce a row that's *worse* than the current all-zero state.
- **`mezase-pokemon-master-rica-matsumoto` may want a yt_id swap as a cosmetic upgrade.** Current yt_id (`hMKf5mE3sdo`) is the THE FIRST TAKE acoustic upload, which has ~30s of spoken intro before the song body. Functionally correct (it IS Mezase by Rica Matsumoto), but the pipeline will produce a misleading "song starts at 30s" highlight unless the intro is trimmed in the timing pipeline OR the yt_id is swapped to the original 1997 OP upload. Defer the swap decision to SEED-001 phase scoping — could be a cheap cosmetic improvement during the same batch.
- **Note (2026-04-26 dupe triage):** the original "yt_id mistakes blocking SEED-001" framing was partially wrong. `guren-no-zahyou-linked-horizon` turned out to NOT be in the broken-bucket — it has LRC + populated verse timing — and was fixable directly via a yt_id swap + offset recalibration. That fix was applied during the SEED-001 planting session. Only `pokemon-getto-da-ze-rica-matsumoto` remains as the dupe-row blocker for SEED-001 (broken-bucket AND wrong yt_id, both blockers).
