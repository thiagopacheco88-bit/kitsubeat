# Milestones: KitsuBeat

## v1.0 — Core Learning Experience (In Progress)

**Goal:** Users can watch anime songs with synced lyrics, furigana, grammar breakdowns, vocabulary, and translations — with auth, catalog, AI search, payments, basic exercises, and Anki export.

**Phases:** 1-6
**Last phase:** 6

| Phase | Name | Status |
|-------|------|--------|
| 1 | Content Pipeline | In Progress (6/8 plans) |
| 2 | Player Experience | Not started |
| 3 | Auth, Catalog, and Freemium Gate | Not started |
| 4 | AI Search and Payments | Not started |
| 5 | Exercises and Gamification | Not started |
| 6 | Export and Polish | Not started |
| 18.1 | UI Internationalization (PT-BR + ES) | Not started |

**Requirements covered:** CONT-01 through CONT-11, PLAY-01 through PLAY-11, USER-01 through USER-06, DISC-01 through DISC-04, MONE-01 through MONE-04, EXER-01 through EXER-04, GAME-01 through GAME-03, EXPO-01 through EXPO-02

---
*Created: 2026-04-14*
