...
---
*Last updated: 2026-05-09 — Phase 14.4 (Virality & Engagement) complete: Migration 0018 applied (3 users cols + activity_events + email_sent_log tables). Data layer: getNowPlayingCounts, getRecentMasteryEvents, applyStreakSaver (grant/consume state machine). Email infra: Resend send helper (dry-run safe), daily-reminder + weekly-recap cron routes (vercel.json with Hobby-plan schedule 0 19 * * *). UI: CoverCard now-playing chip, RecentlyMasteredTicker, StreakSaverToast, ProfileForm social opt-in toggle. ISO week key bug fixed (Thursday-anchor ISO 8601 algorithm). Timezone coverage limitation (UTC-only at 19:00) and vocab count semantics (reviewed vs stage-advanced) accepted as MVP trade-offs.*
